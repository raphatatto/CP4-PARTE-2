...
public class ProdutoController {
    ...
    @PatchMapping("/{id}")
    public ResponseEntity<ProdutoResponse>atulizarParcialmente(
            @PathVariable Long id,
            @RequestBody @Valid ProdutoPatchRequest produtoPatchRequest){
        Optional<ProdutoResponse> produto = produtoService.updatePatch(id, produtoPatchRequest);
        if (produto.isEmpty())
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        return new ResponseEntity<>(produto.get(), HttpStatus.OK);
    }
